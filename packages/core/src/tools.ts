...
import { ToolDefinition, ToolResult, ToolCall, PermissionsConfig, redactSecrets } from '@pulsesparkai/shared';
...
function isUrlBlocked(urlStr: string): string | null {
  let parsed: URL;
  try {
...
async function executeFetchUrl(
  ...
  const blocked = isUrlBlocked(args.url);
  if (blocked) {
