...
import { scanContent, sanitizeForContext, evaluateToolScope, scanToolOutput, scanUserInput } from './content-guard';
...
export class AgentLoop {
  ...
  async processMessage(userMessage: string): Promise<string> {
    const inputScan = scanUserInput(userMessage, this.permissions.max_input_length);
    if (inputScan.truncated) {
      this.handler.showMessage(
        'system',
        `[Input Guard] Message truncated to ${this.permissions.max_input_length} characters`,
      );
    }
    if (!inputScan.safe) {
      this.untrustedContentDetected = true;
      const summary = inputScan.threats.map((t) => `${t.type}: "${t.match}"`).join('; ');
      this.handler.showMessage('system', `[Input Guard] Suspicious patterns in user input: ${summary}`);
    }

    const delimited = `### USER_INPUT_START ###\n${inputScan.normalized}\n### USER_INPUT_END ###`;
    this.messages.push({ role: 'user', content: delimited });
  ...
  private validateToolArgs(call: ToolCall): string | null {
    const a = call.arguments;
    switch (call.name) {
      case 'write_file':
        if (typeof a.path !== 'string' || typeof a.content !== 'string') {
          return 'write_file requires string "path" and "content"';
        }
        break;
      case 'run_command':
        if (typeof a.command !== 'string') {
          return 'run_command requires string "command"';
        }
        break;
      case 'fetch_url':
        if (typeof a.url !== 'string') {
          return 'fetch_url requires string "url"';
        }
        try { new URL(a.url as string); } catch {
          return `fetch_url: invalid URL "${a.url}"`;
        }
        break;
      case 'str_replace':
        if (typeof a.path !== 'string' || typeof a.old_string !== 'string' || typeof a.new_string !== 'string') {
          return 'str_replace requires string "path", "old_string", and "new_string"';
        }
        break;
    }
    return null;
  ...
  private async handleToolCall(call: ToolCall): Promise<{ tool_call_id: string; output: string; error?: string }> {
    ...
    const argError = this.validateToolArgs(call);
    if (argError) {
      return { tool_call_id: call.id, output: '', error: argError };
    }

    const action = this.buildProposedAction(call);

    if (this.handler.showToolStart) {
      this.handler.showToolStart(call.name, action.description);
    }
    ...
    if (needsApproval && !this.shouldAutoApprove(action)) {
      ...
      const approved = await this.requestApprovalWithTimeout(action, diff);
    ...
    const CHAIN_GUARD_WINDOW = 30_000;
    const chainTools = new Set(['write_file', 'str_replace', 'run_command']);
    if (chainTools.has(call.name) && Date.now() - this.lastFetchTimestamp < CHAIN_GUARD_WINDOW) {
      const chainApproved = await this.requestApprovalWithTimeout({
        ...action,
        description: `${action.description} [Chain Guard: write/execute after recent fetch — requires explicit approval]`,
        requires_approval: true,
      });
      if (!chainApproved) {
        return { tool_call_id: call.id, output: '', error: 'Action denied — write/execute blocked after recent fetch' };
      }
    }

    const result = await executeTool(call, this.workspaceRoot, this.permissions);

    if (call.name === 'fetch_url' && !result.error) {
  ...
  private async executeDelegateTask(call: ToolCall): Promise<{ tool_call_id: string; output: string; error?: string }> {
    ...
    const approved = await this.requestApprovalWithTimeout(action);
    if (!approved) {
      return { tool_call_id: call.id, output: '', error: 'Action denied by user' };
    }
  ...
  private async requestApprovalWithTimeout(action: ProposedAction, diff?: string): Promise<boolean> {
    const timeoutMs = this.permissions.approval_timeout_ms;
    if (!timeoutMs || timeoutMs <= 0) {
      return this.handler.requestApproval(action, diff);
    }

    const approval = this.handler.requestApproval(action, diff);
    ...
    if (!result) {
      ...
      this.handler.showMessage('system', `[Approval timed out after ${secs}s — action denied]`);
    }
    return result;
  }
