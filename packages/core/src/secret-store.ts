...
const AUDIT_FILE = 'secret-audit.log';
...
export class SecretStore {
  ...
  constructor(workspaceRoot: string) {
    ...
    this.auditPath = path.join(workspaceRoot, RIVET_DIR, AUDIT_FILE);
  ...
  private audit(action: AuditEntry['action'], key: string): void {
    const entry: AuditEntry = { action, key, timestamp: new Date().toISOString() };
    try {
      this.ensureDir();
  ...
  getAuditLog(): AuditEntry[] {
    try {
      ...
      return lines.filter(Boolean).map((line) => JSON.parse(line) as AuditEntry);
    } catch {
      return [];
    }
  }
